...
public class ThreadTPNS extends Thread {
    ...
    private com.tutk.util.IOTC_GCM_IntentService mIotc_GCM_IntentService;
    ...
    public ThreadTPNS(Context context) {
        mcontext = context;
        mRegistrationId = com.tutk.util.DatabaseManager.s_GCM_token;
        type = TYPE_REGISTER;
        mReg = 0;
    }
                    ...
                    switch (responseCode) {
                        case 200:
                            ...
                            if (entity != null) {
                                ...
                                String appvar = "";
                                if (mcontext != null) {
                                    udid = com.tutk.util.DatabaseManager.uid_Produce(mcontext);
                                    appvar = com.tutk.util.DatabaseManager.get_appver(mcontext);
                                } else {
                                    udid = com.tutk.util.DatabaseManager.uid_Produce(mActivity);
                                    appvar = com.tutk.util.DatabaseManager.get_appver(mActivity);
                                }
                                if (responseBody.indexOf("hi") != -1) {
                                    if (type == TYPE_REGISTER) {
                                        mHttpGetTool("http://" + httpUrl + "/apns/apns.php?cmd=reg_client&token="
                                                + com.tutk.util.DatabaseManager.s_GCM_token + "&appid="
                                                + com.tutk.util.DatabaseManager.s_Package_name + "&dev=0" + "&udid=" + udid + "&os=android"
                                                + "&lang=" + com.tutk.util.DatabaseManager.get_language() + "&osver="
                                                + com.tutk.util.DatabaseManager.get_osver() + "&appver=" + appvar + "&model="
                                                + com.tutk.util.DatabaseManager.get_model());
...
//											DatabaseManager.s_Package_name, "android");
                                        ...
                                        mHttpGetTool("http://" + httpUrl + "/apns/apns.php?cmd=sync&token="
                                                + com.tutk.util.DatabaseManager.s_GCM_token + "&appid="
                                                + com.tutk.util.DatabaseManager.s_Package_name + "&udid=" + udid + "&os=android"
                                                + "&map=" + getUIDList(mcontext));
                                        if (mSupportBaidu && com.tutk.util.IOTC_GCM_IntentService.user_id != null && com.tutk.util.IOTC_GCM_IntentService.channel_id != null) {
                                            mHttpGetTool("http://" + httpUrl + "/apns/apns.php?cmd=reg_client" + "&appid="
                                                    + com.tutk.util.DatabaseManager.s_Package_name_baidu + "&dev=0" + "&udid=" + udid + "&os=baidu"
                                                    + "&token=" + com.tutk.util.IOTC_GCM_IntentService.user_id + "@" + com.tutk.util.IOTC_GCM_IntentService.channel_id + "&lang="
                                                    + com.tutk.util.DatabaseManager.get_language() + "&osver="
                                                    + com.tutk.util.DatabaseManager.get_osver() + "&appver=" + appvar + "&model="
                                                    + com.tutk.util.DatabaseManager.get_model());
                                            mHttpPostsyncApiTool("http://" + httpUrl + "/apns/apns.php", mcontext, udid, com.tutk.util.IOTC_GCM_IntentService.user_id
                                                    + "@" + com.tutk.util.IOTC_GCM_IntentService.channel_id, com.tutk.util.DatabaseManager.s_Package_name_baidu, "android");
                                        }
                                    } else if (type == TYPE_MAPPING) {
                                        if (mReg == MAPPING_UNREG) {
                                            mHttpGetTool("http://" + httpUrl + "/apns/apns.php" + "?cmd=unreg_mapping" + "&appid="
                                                    + com.tutk.util.DatabaseManager.s_Package_name + "&uid=" + mUID + "&udid=" + udid
                                                    + "&os=android", mUID);

                                            if (mSupportBaidu && com.tutk.util.IOTC_GCM_IntentService.user_id != null && com.tutk.util.IOTC_GCM_IntentService.channel_id != null) {
                                                mHttpGetTool("http://" + httpUrl + "/apns/apns.php" + "?cmd=unreg_mapping" + "&appid="
                                                        + com.tutk.util.DatabaseManager.s_Package_name_baidu + "&uid=" + mUID + "&udid=" + udid
                                                        + "&os=baidu" + "&token=" + com.tutk.util.IOTC_GCM_IntentService.user_id + "@"
                                                        + com.tutk.util.IOTC_GCM_IntentService.channel_id, mUID);
                                            }
                                        }
                                        if (mReg == MAPPING_REG) {
                                            ...
                                            if (mSupportBaidu && com.tutk.util.IOTC_GCM_IntentService.user_id != null && com.tutk.util.IOTC_GCM_IntentService.channel_id != null) {
                                                mHttpGetTool("http://" + httpUrl + "/apns/apns.php" + "?cmd=reg_mapping" + "&appid="
                                                        + com.tutk.util.DatabaseManager.s_Package_name_baidu + "&uid=" + mUID + "&udid=" + udid
                                                        + "&os=baidu" + "&token=" + com.tutk.util.IOTC_GCM_IntentService.user_id + "@"
                                                        + com.tutk.util.IOTC_GCM_IntentService.channel_id + "&interval=" + interval);
                                            }
                                        }
                                    }
                                    checkSend = true;
    ...
    public void AddRemoveList() {
        if (mReg == 1) {
            try {
                if (mcontext != null) {
                    com.tutk.util.DatabaseManager manager = new com.tutk.util.DatabaseManager(mcontext);
                    manager.add_remove_list(mUID);
                } else {
                    com.tutk.util.DatabaseManager manager = new com.tutk.util.DatabaseManager(mActivity);
                    manager.add_remove_list(mUID);
                }
            } catch (Exception e) {
                Log.i("tpns", "error" + e.toString());
    ...
    private JSONObject getSyncApiObject(Context mContext, String udid, String token, String appid, String os) {
        ...
        try {
            ...
            Cursor cursor = db.query(com.tutk.util.DatabaseManager.TABLE_DEVICE, new String[]{"_id", "dev_uid"}, null, null, null, null, "_id LIMIT "
    ...
    private String getUIDList(Context context) {
        ...
        com.tutk.util.DatabaseManager manager = new com.tutk.util.DatabaseManager(context);
        SQLiteDatabase db = manager.getReadableDatabase();
        Cursor cursor = db.query(com.tutk.util.DatabaseManager.TABLE_DEVICE, new String[]{"_id", "dev_uid"}, null, null, null, null, "_id LIMIT "
                + MultiViewActivity.CAMERA_MAX_LIMITS);
